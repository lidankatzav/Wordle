...
export function useGame() {
  ...
  useEffect(() => {
    if (gameState.wordOfGame === null) {
      getNewWordToGame().catch(() => alert('Networking Error! Please try again!'));
    }
  }, [gameState]);
  ...
  const checkWord = async (): Promise<void> => {
    ...
    const newColorsLineOfWord = await compareWords(wordToCheck, gameState.wordOfGame).catch((err) => alert('Networking Error! Please try again!'));
